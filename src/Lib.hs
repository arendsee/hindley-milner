...
extend :: Name -> Type -> Map.Map Name Type -> Map.Map Name Type
extend = Map.insert


restrict :: Name -> Map.Map Name Type -> Map.Map Name Type
restrict = Map.delete
...
bind xs (ELam (Arg n t1) e t2)
  = ELam (Arg n t1) (bind xs' e) t2 where
    xs' = extend n (TVar t1) xs
...
apply (EApp e1{-t0-} e2{-t1-} t{-t2-}) = EApp e1''{-(t4->t5)-} e2'{-t4-} t'{-t5-}
  where
    ...
    e1'' = retype t6 e1'
apply (ELam a@(Arg _ t1) e t) = ELam a e' t' where
  e' = apply e
  t' = function (TVar t1) (typeOf e')
